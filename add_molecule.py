...
class MolGen(bpy.types.Operator):
    ...
    def add_atom(self, symbol, x, y, z):  # Adds an atom with the given symbol at coordinates x,y,z.
        if self.hideHydrogen and symbol == "H":
            return
        ...
        # Not strictly necessary, but smooth shading looks better in this case.
        bpy.ops.object.shade_smooth()

    def add_bond(self, a1, a2, n):
        ...
        if self.hideHydrogen and (self.atoms[a1][3] == "H" or self.atoms[a2][3] == "H"):
            return
        ...
        if dx == 0 and dz == 0:
            ...
            rot_x = math.atan(dy / math.sqrt(dz ** 2 + dx ** 2))
        if dz == 0:
            rot_y = math.pi / 2
            if dx > 0:
                rot_y = -rot_y
        else:
            rot_y = math.atan(dx / dz)
        if 0 < dz:
            rot_x = -rot_x
        ...
        for i in range(n):
            ...
            # Finds the straight-line distance between the two atoms on the xy plane.
            dxy = math.sqrt(dy ** 2 + dx ** 2)
            # mod_xy is the distance that this particular bond has to move from the center point of the two atoms.
            mod_xy = self.bondRadius * (2 * i - n + 1) * 2
            ...
            for j in [(1, a1), (-1, a2)]:
                ...
                x = self.atoms[j[1]][0] + dx / 4 * j[0] + mod_x
                y = self.atoms[j[1]][1] + dy / 4 * j[0] + mod_y
    ...
    def execute(self, context):
        ...
        with open(self.filepath) as lines:
            ...
            # atoms_found is a flag used to mark when the file starts declaring the properties of atoms.
            atoms_found = False
            for line in lines:
                # Properties of atoms and bonds in .sdf files are separated by spaces.
                props = line.split()
                ...
                if len(props) >= 2:
                    if props[0] == "M" and props[1] == "END":
                        ...
                        self.add_atom(props[3], float(props[0]), float(props[1]), float(props[2]))
                    elif atoms_found:
                        ...
                        self.add_bond(int(props[0]) - 1, int(props[1]) - 1, int(props[2]))
